...
const ERC4907_RentableService = artifacts.require("ERC4907_RentableService");
 ...
 */
contract("TestRentableService", function (accounts) {
  it("should support the ERC721 and ERC4907 standards", async () => {
    const rentableInstance = await ERC4907_RentableService.deployed();
    const ERC721InterfaceId = "0x80ac58cd";
    const ERC4907InterfaceId = "0xad092b5c";
    var isERC721 = await rentableInstance.supportsInterface(ERC721InterfaceId);
    var isER4907 = await rentableInstance.supportsInterface(ERC4907InterfaceId); 
  ...
  it("should not set UserInfo if not the owner", async () => {
    ...
    await rentableInstance.mint("fakeURI");
    // Failed require in function
    await expectRevert(rentableInstance.setUser(1, accounts[1], expirationDatePast, {from: accounts[1]}), "ERC721: transfer caller is not owner nor approved");
    // Assert no UserInfo for NFT
    var user = await rentableInstance.userOf.call(1);
    var date = await rentableInstance.userExpires.call(1);    
  ...
  it("should return the correct UserInfo", async () => {
    ...
    var expiredTx = await rentableInstance.setUser(2, accounts[1], expirationDatePast)
    var unexpiredTx = await rentableInstance.setUser(3, accounts[2], expirationDateFuture)
    var expiredNFTUser = await rentableInstance.userOf.call(2);
    var expiredNFTDate = await rentableInstance.userExpires.call(2);
    var unexpireNFTUser = await rentableInstance.userOf.call(3);
    var unexpiredNFTDate = await rentableInstance.userExpires.call(3);
    ...
    // Burn NFT
    unexpiredTx = await rentableInstance.burn(3);
    // Assert UserInfo was deleted
    unexpireNFTUser = await rentableInstance.userOf.call(3);
    unexpiredNFTDate = await rentableInstance.userExpires.call(3);
